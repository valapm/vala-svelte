...
import type { BoostPowJobProofModel } from "./boost-pow-job-proof-model"
...
export class BoostPowJobModel {
  ...
  static remainingOperationsMatchExactly(remainingChunks, start: number): boolean {
    ...
    while (i < remainingChunks.length - 8) {
      ...
      if (
        ...
        (remainingChunks[start + i].buf === undefined && expectedOps[i] === remainingChunks[start + i].opcodenum)
      ) {
        i++
      } else {
        return false
  ...
  static fromTransaction(tx: bsv.Transaction, vout: number = 0): BoostPowJobModel | undefined {
    ...
    if (
      tx.outputs[vout].script &&
      tx.outputs[vout].script.chunks[0].buf &&
  ...
  static createRedeemTransaction(
    ...
    receiveAddressStr: string
  ): bsv.Transaction | null {
    ...
    const flags =
      bsv.Script.Interpreter.SCRIPT_VERIFY_MINIMALDATA |
    ...
    unlockingScript
      .add(Buffer.concat([signature.toBuffer(), Buffer.from([sigtype & 0xff])]))
  ...
  static tryValidateJobProof(
    boostPowJob: BoostPowJobModel,
    boostPowJobProof: BoostPowJobProofModel,
    debug: boolean = false
  ): null | { boostPowString: BoostPowStringModel | null; boostPowMetadata: BoostPowMetadataModel | null } {
    ...
    if (debug) {
      ...
      console.log(
        "category",
        boostPowJob.getCategoryBuffer().toString("hex"),
      ...
      console.log(
        "userNonce",
        boostPowJob.getUserNonceBuffer().toString("hex"),
  ...
  static ensure_positive() {
    return [Buffer.from("00", "hex"), bsv.Opcode.OP_CAT, bsv.Opcode.OP_BIN2NUM]
  }
