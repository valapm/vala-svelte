<script lang="ts">
  ...
  const marketStateQuery = gql`
    subscription {
      market_state(where: {state: {transaction: {broadcastFailed: {_eq: false}}}, market: {marketStateByFirststateid: { state: {transaction: {txid: {_eq: "${marketTxid}"}}}}}}, order_by: {stateCount:desc}, limit: 1) {
        totalSatVolume
        accLiquidityFeePool
        liquidityPoints
        liquidityFeePool
  ...
  //   subscription {
  //     market_state(where: {state: {transaction: {broadcasted: {_eq:true}}}, market: {marketStateByFirststateid: { state: {transaction: {txid: {_eq: "${marketTxid}"}}}}}}, order_by: {stateCount:desc}, limit: 1) {
  //       state {
  //         transaction {
  //           txid
  //         }
  ...
  query {
      market(where: {marketStateByFirststateid: { state: {transaction: {txid: {_eq: "${marketTxid}"}}}}}) {
        creatorPubKey
        creatorFee
        details
        version
  ...
  // $: if (marketTxid === "2e5842faa55d9930f024a5963b8cac3720a9927be30a745da1d054e003d38c95") {
...
<main>
  ...
  {:else if market}
    <div class="main-panel">
      ...
      {#if tab === 1}
        <MarketBanner {market} />
        <!-- <a href={`https://${testnet ? "test." : ""}whatsonchain.com/tx/${marketTxid}`} class="txid">
        {marketTxid.slice(0, 20)}...</a
      > -->
        <div class="chart">
          <Chart {market} />
        </div>
